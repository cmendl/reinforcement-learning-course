...
def value_iter_step(tprob, rewards, gamma, u):
    ...
    """
    return rewards + gamma * np.amax(np.tensordot(u, tprob, axes=1), axis=1)
...
def policy_from_utility(tprob, u):
    ...
    """
    return np.argmax(np.tensordot(u, tprob, axes=1), axis=1)
...
def policy_iter_step(tprob, rewards, gamma, pol):
    ...
    """
    u = utility_from_policy(tprob, rewards, gamma, pol)
    return policy_from_utility(tprob, u)
